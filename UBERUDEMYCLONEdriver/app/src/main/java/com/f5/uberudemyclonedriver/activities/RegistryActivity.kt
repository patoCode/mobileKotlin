...
import com.f5.uberudemyclonedriver.databinding.ActivityRegistryBinding
import com.f5.uberudemyclonedriver.providers.Auth
import com.f5.uberudemyclonedriver.providers.Driver
...
class RegistryActivity : AppCompatActivity() {
    ...
    private val driverProvider = Driver()
        ...
        if(validForm(name, lastName, phone, username, password, confirm)){
            authProvider.register(username, password).addOnCompleteListener {
                if(it.isSuccessful){
                    ...
                    driverProvider.create(_client).addOnCompleteListener {
                        if(it.isSuccessful){
